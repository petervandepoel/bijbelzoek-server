...
router.post("/compose/stream", async (req, res) => {
  try {
    ...
    // Prime
    res.write(": ping\n\n");
    console.log("[SSE] start", { mode, model: modelOverride || process.env.OPENROUTER_MODEL, hasKey: !!process.env.OPENROUTER_API_KEY });

    let closed = false;
    req.on("close",  () => { closed = true; console.log("[SSE] client closed"); });
    req.on("aborted", () => { closed = true; console.log("[SSE] client aborted"); });

    let gotFirstToken = false;

    // informeer na 5s
    const infoTimer = setTimeout(() => {
      if (!closed && !gotFirstToken) {
        res.write(`event: info\n`);
        res.write(`data: ${JSON.stringify({ message: "Nog geen tokens ontvangen… controleer model/ENV" })}\n\n`);
      }
    }, 5000);

    // harde fallback na 8s
    let fallbackFired = false;
    const fallbackTimer = setTimeout(async () => {
      if (closed || gotFirstToken || fallbackFired) return;
      fallbackFired = true;
      console.warn("[SSE] fallback → non-stream");
      try {
        const { text, usage } = await callLLM({
          prompt: buildPrompt({ mode, extra, context }),
          model: modelOverride || undefined,
        ...
        if (!closed) {
          ...
          for (const ln of lines) res.write(`data: ${ln.replace(/\r?\n/g, "\\n")}\n\n`);
          res.write(`event: end\n`);
    ...
    await streamLLM({
      ...
      max_tokens: 950,
      onDelta: (chunk) => {
        if (closed || fallbackFired) return;
        if (!gotFirstToken) {
          gotFirstToken = true;
          clearTimeout(infoTimer);
        }
    ...
    console.error("[SSE] route error", e);
    try {
      res.write(`event: error\n`);
      res.write(`data: ${JSON.stringify({ message: e?.message || String(e) })}\n\n`);
      res.end();
...
router.get("/debug/sse", (req, res) => {
  ...
  const t = setInterval(() => {
    ...
    if (i === 5) { clearInterval(t); res.write("event: end\ndata: {\"done\":true}\n\n"); res.end(); }
  }, 1000);
});
