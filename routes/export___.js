...
// simpele wachtrij (max 1 export tegelijk; pas aan naar 2–3 als je wilt)
...
async function renderChart({ base, version, words, mode = "exact", title }) {
  ...
  if (!data.length) return { buffer: null, rows: [], words: list };
  ...
  return { buffer, rows: ranked, words: list };
}
...
async function buildDocxBuffer({ base, generalNotes, favoritesTexts, favoritesCharts, aiResults }) {
  ...
  // ⚠️ Belangrijk: maak het document direct met ÉÉN section (géén lege default)
...
// ───────────────────────────────────────────────────────────────────────────────
// PDF bouwen met Puppeteer (mooie CSS + paginanummering)
// ───────────────────────────────────────────────────────────────────────────────
async function buildPdfBuffer({ base, generalNotes, favoritesTexts, favoritesCharts, aiResults }) {
  ...
  // Charts injecteren (fouttolerant)
    ...
    try {
      ...
      if (png?.rows?.length) {
        ...
        const lines = png.rows
          ...
          .map(
            (r) =>
              `<tr><td>${esc(r.book)}</td>${words
                .map((w) => `<td>${Number(r[w] || 0)}</td>`)
                .join("")}</tr>`
          )
          .join("");
  ...
  try {
    ...
    try { if (page.evaluateHandle) await page.evaluate(async () => document.fonts && (await document.fonts.ready)); } catch {}
    try { if (page.waitForNetworkIdle) await page.waitForNetworkIdle({ idleTime: 100, timeout: 1500 }); } catch {}
    ...
    // ✅ Alleen een minimale sanity-check: niet leeg
    if (!pdf || pdf.length < 800) {
      throw new Error(`PDF buffer te klein (len=${pdf?.length || 0})`);
    }
    return pdf;
  } finally {
    await browser.close();
...
router.post("/:fmt", async (req, res, next) => {
  ...
  try {
    ...
    res.setHeader("Content-Length", String(buf.length)); // ✅ belangrijk
    return res.end(buf);
  } catch (e) {
